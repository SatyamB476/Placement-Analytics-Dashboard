...
df = df[(df["Branch"].isin(selected_branch)) & (df["Year"].isin(selected_year))]
...
with col1:
    ...
    fig1 = px.pie(status_counts, values='Count', names='Status', title='Placement Status', color_discrete_sequence=['#2ecc71', '#e74c3c'])
...
with col2:
    ...
    fig2 = px.bar(branch_rate.sort_values('PlacementRate', ascending=False), x='Branch', y='PlacementRate', title='Placement Rate by Branch (%)', color='PlacementRate', color_continuous_scale='Greens', text='PlacementRate')
...
col1,col2=st.columns(2)
with col1:
    fig=px.box(df,x="PlacementStatus",y="CGPA",color="PlacementStatus",title='CGPA Distribution — Placed vs Not Placed')
...
with col2:
    ...
    fig=px.bar(placementRate_intern,x="Internships",y="PlacementRate",title="Placement Rate of Intern Student",color="PlacementRate")
    st.plotly_chart(fig,use_container_width=True)
col1, col2 = st.columns(2)
with col1:
    ...
    fig=px.bar(backlog_rate,x="Backlogs",y="PlacementRate",title="Backlogs % of Placed Student",color='PlacementRate',text="PlacementRate",color_continuous_scale='Reds_r')
    st.plotly_chart(fig,use_container_width=True)
with col2:
    ...
    fig=px.bar(train_rate,x="Placement_Training",y="training_Rate",color="training_Rate",text="training_Rate",title="Placement Training Placed Rate")
    st.plotly_chart(fig,use_container_width=True)
...
with col1:
    ...
    fig=px.bar(Salary_Branch_Avg,x="Branch",y="Salary_INR",color="Salary_INR",title="Average Salary by Branch")
    st.plotly_chart(fig,use_container_width=True)
with col2:
    total_company_hire=df.dropna(subset=["CompanyName"])["CompanyName"].value_counts().reset_index()
    total_company_hire.columns=["CompanyName","StudentNo"]
    fig=px.bar(total_company_hire,x="CompanyName",y="StudentNo",text="StudentNo",color="StudentNo",title="No of Student Hired by Companies")
    st.plotly_chart(fig,use_container_width=True)
col1, col2 = st.columns(2)
with col1:
    avg_city_salary=df.dropna(subset=["Salary_INR"]).groupby("JobCity")["Salary_INR"].mean().round(2).reset_index()
    avg_city_salary.columns=["JobCity","Salary_INR"]
    fig=px.bar(avg_city_salary,x="JobCity",y="Salary_INR",color="Salary_INR",title="Average salaries in Cities")
    st.plotly_chart(fig,use_container_width=True)
with col2:
    Placed_Cgpa=df[df["PlacementStatus"]=="Placed"]
    fig=px.scatter(Placed_Cgpa,x="CGPA",y="Salary_INR",color="Branch",title="CGPA vs Salary — Placed Students")
    st.plotly_chart(fig,use_container_width=True)
...
with col1:
    ...
    fig=px.box(Salary_Com,x="CommunicationSkill",y="Salary_INR",color="CommunicationSkill",title="Communication Skill vs Salary",category_orders={"CommunicationSkill":["Poor","Average","Good","Excellent"]})
    st.plotly_chart(fig,use_container_width=True)
with col2:
    ...
    fig=px.bar(CGPA_Inter_Realtion,x="Cgpa_Group",color="Internships",y="placeRate",title="CGPA + Internships vs Placement_Rate",barmode="group",category_orders={"Cgpa_Group": ["Low","Medium","High"]})
    st.plotly_chart(fig,use_container_width=True)
    ...
    with st.spinner("Analyzing data..."):
        response = client.chat.completions.create(
           model="llama-3.3-70b-versatile",
            messages=[{"role": "user", "content": f"You are a data analyst. Analyze this placement data and give 3-4 key insights in simple English:\n{summary}"}]
        )
